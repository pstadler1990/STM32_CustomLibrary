...
static uint32_t
MemMap_Flash_ReadWord(uint32_t address)
{
...
static int8_t
MemMap_Flash_ErasePage(uint32_t pageAddress)
{
	uint32_t timeout = FLASH_TIMEOUT;
...
void
MemMap_WriteAllEntriesToFlash(StaticMemoryMap* map, uint32_t size)
{
...
void
MemMap_ReadAllEntriesFromFlash(StaticMemoryMap* map, uint32_t size)
{
	uint32_t sizeOfUint32 = sizeof(uint32_t);
	uint32_t* elemPtr = (uint32_t*)map;
	for(uint32_t elemCnt = 0; elemCnt < size; elemCnt++)
